...
const contactInfo = [];
contactInfo.push(graduates.map(graduates => `${graduates.first_name} ${graduates.email}`));
...
const uni = [];
let search = /Uni/g;
uni.push(graduates.filter(graduates => graduates.university.match(search)));
console.log(uni);
...
const lowerCase = [];
lowerCase.push(zooAnimals.map(zooAnimals => zooAnimals.animal_name.toLowerCase()));
...
const populationTotal = zooAnimals.reduce((acc, zooAnimals) => {return acc + zooAnimals.population}, 0);
